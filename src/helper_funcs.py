...
MAX_RANGE = 1000
ATTRIBUTE_CACHE_VERSION = "kmeans_rbf_anchor_v6"
...
CPP_INDEX_BUILD_TAG = "v2_postm32_efc400"

# Tau curves estimated from src/out/tau_selected/{exp8,exp9,exp10,exp11}.
TAU_SELECTIVITY_POINTS = np.array([0.001, 0.01, 0.1, 0.5], dtype=np.float64)
TAU_POINTS_BY_METRIC = {
    "true": np.array(
        [0.08590421087539804, 0.03800683997329563, 0.04530333608573103, 0.0192882428451402],
        dtype=np.float64,
    ),
    "heuristic": np.array(
        [0.10438473561764555, 0.10438473561764555, 0.10438473561764555, 0.10438473561764555],
        dtype=np.float64,
    ),
    "tanh": np.array(
        [0.007353511740477506, 0.028363545284698952, 0.026088259737914486, 0.006373590291216803],
        dtype=np.float64,
    ),
    "pgap": np.array(
        [0.0, 0.0013520660140296991, 0.004520772212549714, 0.003753587372749595],
        dtype=np.float64,
    ),
}


def interpolated_metric_tau(selectivity, metric):
    ...
    tau_points = TAU_POINTS_BY_METRIC[metric_key]
    log_points = np.log10(TAU_SELECTIVITY_POINTS)
    log_s = np.log10(s)
    tau = np.interp(
        log_s,
        log_points,
...
def assign_smoothed_random_anchors(
    centroids: np.ndarray,
    seed: int = 0,
    smoothing_steps: int = 12,
    keep_ratio: float = 0.2,
    knn: int | None = None,
) -> np.ndarray:
    """Assign locally smooth but globally non-trivial anchors in [0, 1].

    Nearby centroids receive similar anchor values via graph smoothing, but we
    intentionally avoid imposing a single global PCA ordering. That keeps the
    metadata correlated with locality without making the dataset trivially
    separable along one dominant axis.
    """
    ...
    if k == 1:
        print("Smoothed anchor assignment | k=1, assigning anchor [0.0]")
        return np.array([0.0], dtype=np.float32)

    rng = np.random.default_rng(seed)
    raw = rng.random(k, dtype=np.float32)

    centroid_dists = cdist(centroids, centroids, metric="euclidean").astype(np.float32)
    np.fill_diagonal(centroid_dists, np.inf)

    if knn is None:
        knn = max(3, min(k - 1, int(np.ceil(np.sqrt(k)))))
    else:
        knn = max(1, min(k - 1, int(knn)))

    nearest = np.min(centroid_dists, axis=1)
    sigma = max(float(np.median(nearest)), 1e-12)
    print(
        "Smoothed anchor assignment | "
        f"knn={knn} | centroid_sigma={sigma:.6f}"
    )

    neighbor_idx = np.argpartition(centroid_dists, kth=knn - 1, axis=1)[:, :knn]
    affinity = np.zeros((k, k), dtype=np.float32)
    for cid in range(k):
        nbrs = neighbor_idx[cid]
        nbr_dists = centroid_dists[cid, nbrs]
        weights = np.exp(-(nbr_dists ** 2) / (2.0 * sigma ** 2)).astype(np.float32)
        affinity[cid, nbrs] = weights
        affinity[cid, cid] = 1.0
    ...
    anchors = raw.copy()
    for _ in range(int(smoothing_steps)):
        anchors = (1.0 - float(keep_ratio)) * (affinity @ anchors) + float(keep_ratio) * raw

    a_min = float(anchors.min())
    a_max = float(anchors.max())
    if a_max > a_min:
        anchors = (anchors - a_min) / (a_max - a_min)
    else:
        anchors = np.linspace(0.0, 1.0, k, dtype=np.float32)

    print(f"Raw centroid anchors   | {_format_preview(raw)}")
    print(f"Smoothed centroid anchors | {_format_preview(anchors)}")
    return anchors


def estimate_local_rbf_sigma(
    vecs: np.ndarray,
    centroids: np.ndarray,
    sample_size: int,
    seed: int,
    ...
    """Estimate sigma from local centroid spacing instead of all-centroid distances."""
    rng = np.random.default_rng(seed)
    n_samples = len(vecs)
    ...
    if sample_dists.shape[1] == 1:
        local_scale = sample_dists[:, 0]
    else:
        nearest_two = np.partition(sample_dists, kth=1, axis=1)[:, :2]
        local_scale = nearest_two[:, 1]

    sigma = float(np.median(local_scale) * float(sigma_scale))
    sigma = max(sigma, 1e-12)
    print(
        "Local sigma estimate | "
        f"median_local_scale={float(np.median(local_scale)):.6f} | sigma={sigma:.6f}"
    )
    return sigma
...
def generate_correlated_attribute(
    ...
    """Assign correlated integer attributes via a smoothed centroid field."""
    ...
    anchors = assign_smoothed_random_anchors(centroids, seed=seed)

    print("Estimating sigma from a random subsample...")
    sigma = estimate_local_rbf_sigma(
        vecs=vecs,
        centroids=centroids,
        sample_size=10000,
        seed=seed,
    ...
    attr_cont = np.zeros(len(vecs), dtype=np.float32)
    batch_size = 10000
...
def ensure_cpp_index(attr, tag="correlated", base_fvecs_path=None):
    _ensure_cpp_tools()
    out_dir = Path("/tmp/fvs_cache/cpp_indexes") / f"{tag}_{CPP_INDEX_BUILD_TAG}"
    out_dir.mkdir(parents=True, exist_ok=True)

    attr_path = out_dir / "attr.npy"
    np.save(attr_path, np.asarray(attr, dtype=np.int32))

    base_path = Path(base_fvecs_path) if base_fvecs_path is not None else BASE_FVECS_PATH
